...
- Every `actions/upload-artifact` in `build.yaml`/`build-github.yaml` is a 3-attempt chain (`continue-on-error` + `sleep` 60/120, `overwrite: true` on retries). A DNS blip on the artifact API once discarded a finished 70-minute darwin/arm64 build (`Failed to CreateArtifact: ENOTFOUND`) — by that step the build is already hours deep, so never collapse the chain back to a single upload.
